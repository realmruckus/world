...
function weightedScore(rule, metrics) {
  let score = 0;
  for (const [id, weight] of Object.entries(rule.score || {})) {
    if (!(id in metrics)) throw new Error(`Unknown ending metric: ${id}`);
    score += metrics[id] * weight;
  }
  return score;
}
...
export function resolveEnding(life, metrics, rules) {
  ...
  const lifeScore = Math.round(Object.entries(rules.lifeScoreWeights || {}).reduce((sum, [id, weight]) => {
    if (!(id in metrics)) throw new Error(`Unknown ending metric: ${id}`);
    return sum + metrics[id] * weight;
  }, 0));
